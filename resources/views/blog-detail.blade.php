...
<section class="relative z-10 pt-32 pb-10 md:pt-40 md:pb-14 text-white">
  <!-- <div class="pt-32 pb-16 md:pt-40 md:pb-20"> -->
  ...
  <div aria-hidden="true"
    ...
    <img src="{{ asset('image/beranda/pattern-3_29_11zon.jpg') }}"
      ...
      class="hidden md:block absolute left-0 top-10 md:top-0 w-[240px] md:w-[280px] h-auto opacity-30 select-none">

    {{-- pola kiri bawah --}}
    <img src="{{ asset('image/beranda/pattern-4_30_11zon.jpg') }}"
      alt="pattern garis kotak bawah"
      class="hidden md:block absolute left-0 bottom-1 md:bottom-1 w-[240px] md:w-[280px] h-auto opacity-30 select-none">
  ...
  <div class="container-lg max-w-5xl">
    ...
    <div class="mb-6">
      <a href="{{ route('blog.index') }}"
        class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/20
            bg-white/10 text-xs md:text-sm text-white/80
            hover:bg-sky-500/90 hover:text-white transition">
        <span class="text-base md:text-lg">&larr;</span>
    ...
    {{-- HEADER ARTIKEL --}}
    <article
      class="rounded-[32px] bg-slate-950/80 
      border border-transparent
      hover:border-sky-500/90
      transition-colors duration-300
      px-5 py-6 md:px-9 md:py-8 shadow-xl/40 ">
      <header class="mb-6 md:mb-8">
        <div class="flex flex-wrap items-center gap-3 mb-3">
          {{-- kategori sebagai chip --}}
          <span
            class="inline-flex items-center rounded-full bg-[var(--color-accent)]/90 text-white px-3 py-1 text-[11px] font-semibold">
            {{ $article->category_label }}
          </span>

          {{-- tanggal --}}
          <div class="inline-flex items-center gap-2 rounded-full bg-sky-500/90 text-white
         px-3 py-1 text-[11px] md:text-xs font-medium shadow-sm">
            <x-heroicon-o-calendar class="w-4 h-4" />
            <span>{{ $article->date }}</span>
          </div>
        </div>

        <h1 class="text-2xl md:text-3xl lg:text-[32px] font-display text-white leading-tight mb-3">
          {{ $article->title }}
        </h1>

        @if(!empty($article['excerpt']))
        <p class="text-sm md:text-base text-white/75 max-w-3xl">
          {{ $article->excerpt }}
        </p>
        @endif
      </header>
    ...
    <section class="mt-10 md:mt-12">
      ...
      <div class="grid gap-4 md:grid-cols-3">
        ...
        <a href="{{ route('blog.show', $rel->slug) }}"
          class="group rounded-2xl bg-slate-900/70 border border-white/10 hover:border-[var(--color-accent)]/80 transition overflow-hidden flex flex-col">
          <div class="h-28 md:h-24 overflow-hidden">
            <img src="{{ asset($rel->image_url) }}"
              alt="{{ $rel->title }}"
              class="w-full h-full object-cover group-hover:scale-[1.03] transition-transform">
          </div>
          <div class="p-4 flex-1 flex flex-col">
            <div class="text-[10px] md:text-[11px] text-white/60 mb-1">
              {{ $rel->date }}
            </div>
            <p class="text-xs md:text-sm font-semibold text-white leading-snug line-clamp-3">
              {{ $rel->title }}
            </p>
          </div>
        </a>
