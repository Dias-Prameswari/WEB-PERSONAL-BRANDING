<x-app-layout>
    ...
    <div class="py-12">
        <div class="container-lg">
            <div class="rounded-2xl bg-white/95 shadow-lg p-5">
                @if(session('ok'))
                ...
                <div class="flex flex-col md:flex-row md:items-center gap-3 mb-4">
                    ...
                    <a href="{{ route('admin.portofolio.create') }}" class="btn-primary whitespace-nowrap">
                        + Proyek Baru
                    </a>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 text-gray-700">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-left font-semibold">Tech</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-t hover:bg-gray-50/70">
                                ...
                                <td class="px-3 py-2">{{ $p->category_slug ?? '-' }}</td>
                                <td class="px-3 py-2">
                                    @if(is_array($p->techs))
                                    {{ implode(', ', $p->techs) }}
                                    @else
                                ...
                                <td class="px-3 py-2">{{ $p->published ? 'Published' : 'Draft' }}</td>
                                <td class="px-3 py-2 text-right space-x-2">
                                    <a href="{{ route('admin.portofolio.edit', $p) }}"
                                        class="text-blue-600 text-xs underline">Edit</a>

                                    <form action="{{ route('admin.portofolio.destroy', $p) }}"
                                        method="POST"
                                        class="inline"
