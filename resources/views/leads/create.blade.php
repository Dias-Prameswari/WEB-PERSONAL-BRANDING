      ...
      <div>
        ...
        <select name="program" class="w-full border rounded-md">
          <option value="">Pilih salah satu…</option>
            <option @selected(old('program')==='Newsletter')>Newsletter</option>
            <option @selected(old('program')==='Pelatihan Branding')>Pelatihan Branding</option>
            <option @selected(old('program')==='Konsultasi 1:1')>Konsultasi 1:1</option>
        </select>
      </div>

      <div>
        <label class="block text-sm mb-1">Pesan (opsional)</label>
        <textarea name="message" rows="3" class="w-full border rounded-md">{{ old('message') }}</textarea>
      </div>
